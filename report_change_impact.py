...
def _get_tags_for_issues(issues, tags, target_branch):
    """Retrieves the tags (i.e. the contents of the "Fix for" field)
    of the DMS issues in the `issues` list, filtering them by the tags
    in the `tags` list. The return value is a dictionary mapping each
    issue to its corresponding tag. If an issue doesn't have a tag
    it's omitted from the resulting dictionary.
    ...
    result = {}
    tag_map = server_conn.dms_for_tags(",".join(issues),
                                       ",".join(tags),
                                       target_branch)
    for tag, issues in tag_map.iteritems():
        if issues:
...
def _get_dms_violations(config, dmslist, affected_manifests):
    ...
    if dmslist:
        all_tags = []
        for branchpolicy in config.branches:
            all_tags.extend(branchpolicy["tagnames"])
        for branch in filter(config.branch_has_policy,
                             affected_manifests):
            try:
                issue_tags = _get_tags_for_issues(dmslist, all_tags, branch)
            except DMSTagServerError, e:
                semcutil.fatal(1, "DMS tag server error: %s" % e)
            ok = True
            for issue in dmslist:
                if issue in issue_tags and \
                        config.is_tag_allowed(issue_tags[issue],
                                              branch):
                    pass
                else:
                    ok = False
            ...
            if not ok:
                ...
                if tagnames:
                    ...
                    violations.append(MESSAGE_DMS_REQUIRED % branch)
    else:
        for branch in filter(config.branch_has_policy,
                             affected_manifests):
